<template>
    <header style="background: #872c5d;;">
        <div id="HD_Img_Container">
            ...
            <h1><router-link style="color: white;" to="/ctg/reg_FleaMarket">CONTIGO</router-link></h1>
        </div>

             <ul class="nav" id="header-nav1">
                <li><router-link style="color: white;" to="/ctg/reg_FleaMarket">플리마켓</router-link></li>
                <li><router-link style="color: white;" to="/ctg/reg_FleaMarket">공지사항</router-link></li>
                <li><router-link style="color: white;" to="/ctg/reg_FleaMarket">문의글</router-link></li>
                <!-- 로그인아이디가 있으면 보여진다. -->
                 <!-- v-if="!$store.state.account.member" -->
                 <!-- v-if="$store.state.bool == false" -->
               
                <div class="rightNav" v-if="!$store.state.bool">
                    <li><router-link style="color: white;" to="/user/signIn">로그인</router-link></li>
                    <li><router-link style="color: white;" to="/user/signUp">회원가입</router-link></li>
                </div>
                <div class="rightNav" v-else>
                        <li><router-link style="color: white;" to="/user/signIn" @click="Logout_Ctg_Member">로그아웃</router-link></li>
                        <li style="color: white;">내정보 이름주소 추가하기</li>
                </div>
            </ul>
           <hr>
  </header>
